...
public class VehicleControl : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        InvokeRepeating("CarSound", 0f, 0.1f);
    ...
    public void CarSound()
    {
                if (carEngineSound != null)
                {
                    float engineSoundPitch = initialCarEngineSoundPitch + (Mathf.Abs(myRigidbody.velocity.magnitude) / 25f);
                    carEngineSound.pitch = engineSoundPitch;
                }
                if ((isDrifting) && Mathf.Abs(speed) > 12f)
                {
                ...
                else if ((!isDrifting) || Mathf.Abs(speed) < 12f)
                {
